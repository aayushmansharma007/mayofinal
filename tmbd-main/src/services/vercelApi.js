...
// Fetch anime by different categories using Vercel API routes
...
export const fetchAdultContent = async () => {
  try {
    // Use popular endpoint for adult content as fallback
    const data = await rateLimitedFetch(`${BASE_URL}/popular`, 'adult_anime');
    const filteredShows = filterAnimeContent(data.results || []);
...
// Search anime using Vercel API route
...
export const fetchFallbackAnime = async () => {
  try {
    // Try to get popular anime as fallback
    const data = await rateLimitedFetch(`${BASE_URL}/popular`, 'fallback_anime');
    const filteredShows = filterAnimeContent(data.results || []);
    
    // If we still don't have enough content, return manual fallback
